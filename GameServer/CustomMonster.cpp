...
void CCustomMonster::Load(char* path) // OK
{
	...
	try
	{
		while(true)
		{
			...
			info.SummonMonster2 = lpMemScript->GetAsNumber();

			info.SummonMonsterCount2 = lpMemScript->GetAsNumber();

			info.SummonMonster3 = lpMemScript->GetAsNumber();

			info.SummonMonsterCount3 = lpMemScript->GetAsNumber();
...
void CCustomMonster::MonsterDieProc(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
		...
		if(CustomMonsterInfo.SummonMonster > 0)
	{
		...
		//New Custom Monster 2
		if(CustomMonsterInfo.SummonMonster2 > 0)
	{
		int qtd = (CustomMonsterInfo.SummonMonsterCount2 > 0) ? CustomMonsterInfo.SummonMonsterCount2 : 1;
		...
		for(int n=0;n < qtd;n++)
		{
			...
			if(gObjSetMonster(index,CustomMonsterInfo.SummonMonster2) == 0)
			{
				gObjDel(index);
			}
		}
		...
		//New Custom Monster 3
		if(CustomMonsterInfo.SummonMonster3 > 0)
	{
		int qtd = (CustomMonsterInfo.SummonMonsterCount3 > 0) ? CustomMonsterInfo.SummonMonsterCount3 : 1;
		...
		for(int n=0;n < qtd;n++)
		{
			...
			if(gObjSetMonster(index,CustomMonsterInfo.SummonMonster3) == 0)
			{
				gObjDel(index);
			}
		}
