...
bool gObjSetMonster(int aIndex,int MonsterClass) // OK
{
	...
	if(lpObj->AttackType != 0)
	{
		...
		if(MonsterClass == 419)
		{
			gSkillManager.AddSkill(lpObj,SKILL_PHOENIX_SHOT,0);
			gSkillManager.AddSkill(lpObj,SKILL_DRAGON_SLAYER,0);
		}
		//Ferea Archer
		if(MonsterClass == 569)
		{
		...
		if(MonsterClass == 27)
		{
			gSkillManager.AddSkill(lpObj,SKILL_METEORITE,0);
			gSkillManager.AddSkill(lpObj,SKILL_PLASMA_STORM,0);
		}	
		//General Ferea
		if(MonsterClass == 613)
		{
			gSkillManager.AddSkill(lpObj,SKILL_SPIRAL_SLASH,0);
			gSkillManager.AddSkill(lpObj,SKILL_IMPALE,0);
		}
		//Monster of Water	
		if(MonsterClass == 622)
		{
			gSkillManager.AddSkill(lpObj,SKILL_TWISTER,0);
			gSkillManager.AddSkill(lpObj,SKILL_ICE_STORM,0);
		}
		//God of Darkness
		if(MonsterClass == 623)
					...
					if(lpObj->IsBot >= 1)
					{
						switch(lpObj->IsBot){
							case 2:		//Bot Alchemist
								if ( !(rand()%70) ){
									ChatSend(lpObj,gMessage.GetMessage(873));
