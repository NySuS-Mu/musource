...
-- Tabla de manejadores (callbacks) para OnUserDamage
local OnUserDamage_Handles = {}

function OnUserDamage(aIndex, bIndex, Damage, Skill, Combo, DmgFlag)
    for i = 1, #OnUserDamage_Handles do
        local newDamage = OnUserDamage_Handles[i].callback(aIndex, bIndex, Damage, Skill, Combo, DmgFlag)
...
-- Función para registrar scripts en OnUserDamage
function GameServerFunctions.OnUserDamage(callback)
    table.insert(OnUserDamage_Handles, { callback = callback })
end
