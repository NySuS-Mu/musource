...
void CServerInfo::ReadCommonInfo() // OK
{
	...
	{	
		gMessage.Load(gPath.GetFullPath("Util\\Translate\\Message_eng.txt"));
	return;
	}

	if (gServerInfo.m_ServerLanguage == 1)	
	{	
		gMessage.Load(gPath.GetFullPath("Util\\Translate\\Message_spn.txt"));
	return;
	}

	if (gServerInfo.m_ServerLanguage == 2)	
	{	
		gMessage.Load(gPath.GetFullPath("Util\\Translate\\Message_por.txt"));
	return;
	}
...
void CServerInfo::ReloadBotAlchemistInfo() // OK
{
	BotAlchemist.Read(gPath.GetFullPath("Custom\\BotSystem\\BotAlquimista.txt"));
	BotAlchemist.MakeBot();
	LogAdd(LOG_GREEN,"[ReadBotInfo] BotSystem [alquimista] loaded successfully");
}
void CServerInfo::ReloadBotInfo() // OK
{
	...
	ObjBotBuff.MakeBot();
#endif
	LogAdd(LOG_GREEN,"[ReadBotInfo] BotSystem [buffer] loaded successfully");
}
//MC bot
...
void CServerInfo::ReadCashShopInfo(char* section,char* path) // OK
{
	...
	this->m_CashShopGoblinPointValue[0] = GetPrivateProfileInt(section,"CashShopGPointValue_AL0",0,path);
	this->m_CashShopGoblinPointValue[1] = GetPrivateProfileInt(section,"CashShopGPointValue_AL1",0,path);
	this->m_CashShopGoblinPointValue[2] = GetPrivateProfileInt(section,"CashShopGPointValue_AL2",0,path);
	this->m_CashShopGoblinPointValue[3] = GetPrivateProfileInt(section,"CashShopGPointValue_AL3",0,path);
...
void CServerInfo::ReadCommonInfo(char* section,char* path) // OK
{
	...
	this->m_NewCloseServer = GetPrivateProfileInt(section,"NewCloseServer",0,path);
...
void CServerInfo::ReadCustomInfo(char* section,char* path) // OK
{
	...
	//Lucky Wheel
	this->LuckyWheelMoney = GetPrivateProfileInt(section,"LuckyWheelWcoinsValue",0,path);
}
