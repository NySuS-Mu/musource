		...
		if(gServerInfo.m_CashShopCoinsDelay >= 1){
            if((GetTickCount()-lpObj->CashShopGoblinPointTime) >= ((DWORD)gServerInfo.m_CashShopCoinsDelay*60000)){
                ...
                if(gServerInfo.m_CashShopWCoinCValue[lpObj->AccountLevel] >= 1){
                    gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(870),gServerInfo.m_CashShopWCoinCValue[lpObj->AccountLevel]);
                }
                if(gServerInfo.m_CashShopWCoinPValue[lpObj->AccountLevel] >= 1){
                    gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(871),gServerInfo.m_CashShopWCoinPValue[lpObj->AccountLevel]);
                }
                if(gServerInfo.m_CashShopGoblinPointValue[lpObj->AccountLevel] >= 1){
                    gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(872),gServerInfo.m_CashShopGoblinPointValue[lpObj->AccountLevel]);
                }
            }
        }
